    ...
    public void makeStrObject(String dtoClassName, String springPackageName, Long apiId, String dir)
        ...
        if (apiRequests.isEmpty()) {
            ...
            for (ApiRequest apiRequest : apiRequests) {
                //필드네임 정하기
                String fieldName =
                    apiRequest.getKey().substring(0, 1).toLowerCase() + apiRequest.getKey()
                        .substring(1);
                ...
                if (apiRequest.getType().equals("Object")) {
                    //클래스 이름 정하기
                    String className =
                        apiRequest.getKey().substring(0, 1).toUpperCase() + apiRequest.getKey()
                            .substring(1);
                    ...
                    List<Map<String, Object>> dataMap =
                        new ObjectMapper().readValue(apiRequest.getData(),
